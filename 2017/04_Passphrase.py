...
def valid_passphrase(phrase):
    row = phrase.strip().split()
    return len(row)==list(set(row))

passphrases = input.strip().split('\n')
sum([1 for x in passphrases if valid_passphrase(x)])

# With anagrams
def especially_valid_passphrase(phrase):
    row = [''.join(sorted(word)) for word in phrase.strip().split()]
    return len(row)==list(set(row))

passphrases = input.strip().split('\n')
sum([1 for x in passphrases if especially_valid_passphrase(x)])
